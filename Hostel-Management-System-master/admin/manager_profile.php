...
// Initialize variables for Hostel Manager info
$Fname = $Lname = $Uname = $Mob = $Hostel_name = $Mess_name = $Hostel_Occupancy = $Mess_Occupancy = "Not set";

// Check if session exists
if(isset($_SESSION['hostel_man_id'])){
    $Man_id = $_SESSION['hostel_man_id'];
    $query1 = "SELECT * FROM hostel_manager WHERE Hostel_man_id = '".mysqli_real_escape_string($conn,$Man_id)."'";
    $result = mysqli_query($conn, $query1);

    if($result && mysqli_num_rows($result) > 0){
        $row = mysqli_fetch_assoc($result);
        $Fname = $row['Fname'] ?? "Not set";
        $Lname = $row['Lname'] ?? "Not set";
        $Uname = $row['Username'] ?? "Not set";
        $Mob = $row['Mob_no'] ?? "Not set";
        $Hostel_name = $row['Hostel_id'] ?? "Not set";
        $Mess_name = $row['Mess_id'] ?? "Not set";
        $Hostel_Occupancy = $row['Hostel_Occupancy'] ?? "Not set";
        $Mess_Occupancy = $row['Mess_Occupancy'] ?? "Not set";
    }
}
$adFname = $adLname = $adUname = $adMob = "Not set";
$queryA = "SELECT * FROM Hostel_Manager WHERE Isadmin = 1 LIMIT 1";
$resultA = mysqli_query($conn, $queryA);
if($resultA && mysqli_num_rows($resultA) > 0){
    $rowA = mysqli_fetch_assoc($resultA);
    $adFname = $rowA['Fname'] ?? "Not set";
    $adLname = $rowA['Lname'] ?? "Not set";
    $adUname = $rowA['Username'] ?? "Not set";
    $adMob = $rowA['Mob_no'] ?? "Not set";
}
	...
	<header>
		<div class="container agile-banner_nav">
			<nav class="navbar navbar-expand-lg navbar-light bg-light">
				...
				<h1><a class="navbar-brand" href="../home_manager.php">SUSL <span class="display"></span></a></h1>
...
<fieldset class="step agileinfo w3ls_fancy_step">
    ...
    <div class="abt-agile">
        ...
        <div class="abt-agile-right">
            <!-- USE THE VARIABLES SET ABOVE, DON'T RE-RUN QUERY -->
            <h3><?php echo $Fname." ".$Lname; ?></h3>
            <h5>Hostel Manager</h5>
            <ul class="address">
                    <li><b>Username:</b> <?php echo $Uname; ?></li>
                    <li><b>Phone:</b> <?php echo $Mob; ?></li>
                    <li><b>Managing Hostel:</b> <?php echo $Hostel_name; ?></li>
                    <li><b>Managing Mess:</b> <?php echo $Mess_name; ?></li>
                    <li><b>Hostel Occupancy:</b> <?php echo $Hostel_Occupancy; ?>%</li>
                    <li><b>Mess Occupancy:</b> <?php echo $Mess_Occupancy; ?>%</li>
                </ul>
        </div>
        <div class="clear"></div>
    </div>
